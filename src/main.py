...
def write_summary(charErrorRates, wordAccuracies):
    with open(FilePaths.fnSummary, "w") as f:
        json.dump(
            {"charErrorRates": charErrorRates, "wordAccuracies": wordAccuracies}, f
        )
...
def train(model, loader):
    ...
    noImprovementSince = (
        0  # number of epochs no improvement of character error rate occured
    )
...
def main():
    ...
    # infer text on test image
        ...
        model = Model(
            open(FilePaths.fnCharList).read(),
            decoderType,
            mustRestore=True,
            dump=args.dump,
        )
